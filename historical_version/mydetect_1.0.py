            ...
            if not queue.empty():
                ...
                while True:
                    ...
                    while True:
                        T = time.time()
                        # res = ser.read()
                        res = keyboard.hook(pressed_keys)
                        if res:
                            # res = res.decode()
                            #print("res", res)
                            if res == '1':
                ...
                # res_=ser.read()
                # res_ = res_.decode()
                res_ = keyboard.hook(pressed_keys)
...
class yolov5_detect():
    def __init__(self ,weights='best.pt', device='0', conf_thres=0.5, iou_thres=0.45):
        super().__init__()
