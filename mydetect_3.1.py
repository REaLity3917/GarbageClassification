...
FILE = Path(__file__).resolve()#获取当前目录(detect.py)的(使用relsove)绝对路径,并将其赋值给变量FILE
ROOT = FILE.parents[0]  # YOLOv5 root directory 获取上一级目录
...
class CameraUI(QWidget):
    def __init__(self):
        ...
        #串口
        # ser_process = multiprocessing.Process(target=mykeyboard, args=(self.queue, self.shared_value, self.shared_value2,self.shared_load,))      #调试：键盘
            ...
            if not queue.empty():
                ...
                for (x0, y0, x1, y1) in waste_coor:
                    ...
                    if x_bf-x<15 and y_bf-y<15:
                        ...
                        if (x1 - x0) > (y1 - y0):
                            ac = random.choice(['x','w'])
                            y += (-rand if y > 300 else rand)
                        else:
                            ac = random.choice(['y','z'])
                            x += (-rand if x > 300 else rand)
                        print("x,y:",x,y)
                    else:
                        x_bf, y_bf = x, y
                        if (x1-x0)>(y1-y0):
