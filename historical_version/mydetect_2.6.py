...
class CameraUI(QWidget):
    ...
    def change_txt(self):
        ...
        if len(self.detect_process.retu) < len(self.last_wastes):
            ...
            if 'kitchen' in a:
                self.kk += 1
                self.retxt2.append(f'{self.mi}       厨余垃圾        {self.kk}        OK!\n')
            elif 'hazardous' in a:
                self.hh += 1
                self.retxt2.append(f'{self.mi}       有害垃圾        {self.hh}        OK!\n')
            elif 'recyclable waste' in a:
                self.rr += 1
                self.retxt2.append(f'{self.mi}       可回收垃圾      {self.rr}        OK!\n')
            elif 'other waste' in a:
                self.ee += 1
                self.retxt2.append(f'{self.mi}       其他垃圾        {self.ee}        OK!\n')
            # self.rr, self.ee, self.kk, self.hh = 0, 0, 0, 0
        print('retxt2', self.retxt2)
        text = ''
        for length in range(len(self.retxt2)):
            if self.retxt2[length] != '':
                text += self.retxt2[length]
        self.report1.setText(text)
